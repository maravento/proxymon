...
MyDie("invalid user")  unless defined $user  && $user  =~ /^[A-Za-z0-9._\@-]+$/ && $user !~ /\.\./;
$mode = "day" unless defined $mode && $mode =~ /^(day|month|year)$/;

InitTPL("user_detail", scalar $co->param('tpl'));
...
foreach $workday (sort @datelist) {
  ...
  open(my $ffUser, '<', "$workday/$user") or MyDie("can't open $workday/$user");
  $totalsize=<$ffUser>;chomp $totalsize;$totalsize=~s/total: //;
  while (<$ffUser>) {
    ($site,$size,$hit)=split;
    $h{$site}{hit} +=$hit;
    $h{$site}{size}+=$size;
...
# only in DAY mode
if ($mode eq "day") {
  if (open(my $ffBig, '<', "$reportpath/$year$month$day/.bigfiles")) {
    while (<$ffBig>) {
